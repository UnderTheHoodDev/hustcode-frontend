...
import useUpdateProblem from '@/lib/api/problem/mutations/use-update-problem';
...
type EditProblemFormData = z.infer<typeof editProblemSchema>;
...
const EditProblemModal = ({
  ...
  const form = useForm<EditProblemFormData>({
    resolver: zodResolver(editProblemSchema) as any,
    mode: 'onSubmit',
    defaultValues: {
      title: '',
      description: '',
  ...
  const problemDetail = problemDetailResponse?.data as
    | AdminProblemDetail
    | undefined;
  ...
  // Reset form when problem detail is loaded
  React.useEffect(() => {
    if (problemDetail && open) {
      // Map testcases from API response
      const testcases =
        problemDetail.testcases?.map((tc) => ({
          input: tc.input,
          output: tc.output,
      ...
      form.reset({
        ...
        tags: problemDetail.tags.map((t) => t.name).join(', '),
        timeLimit: problemDetail.problemConstrain?.timeLimit || 1000,
        memoryLimit: problemDetail.problemConstrain?.memoryLimit || 256,
        testcases: testcases,
        solution: problemDetail.solution
          ? {
              code: problemDetail.solution.code,
              languageId: problemDetail.solution.languageId,
            }
          : {
              code: '',
              languageId: '',
  ...
  }, [problemDetail, open, form, replace]);

  const onSubmit = (data: EditProblemFormData) => {
    const payload: UpdateProblemDto = {
      ...
      // Include solution if code is provided
      ...(data.solution?.code && {
        solution: {
  ...
  // Loading state
  if (isLoadingDetail && open) {
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="border-[#3a4556] bg-[#1e293b] sm:max-w-2xl">
        ...
        <DialogContent className="border-[#3a4556] bg-[#1e293b] sm:max-w-2xl">
          <div className="flex h-64 flex-col items-center justify-center">
            ...
            <Button
              ...
              className="mt-4 border-[#3a4556] text-gray-300"
            >
              Close
            </Button>
          </div>
